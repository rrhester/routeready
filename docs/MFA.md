...
## Server-side enforcement (closing the API bypass) — migration 0540
...
and never see the challenge (launch-audit finding H-1). Migration `0540` adds the
...
`0540` protects the settings mutators as the pattern. Broadly closing the API — reads of
driver PII, the privileged writes gated in `0539`, etc. — means adding
...
above if you want them gone). Server enforcement (0540) is separately reversible by
