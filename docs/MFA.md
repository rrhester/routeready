...
## Server-side enforcement (closing the API bypass) — migration 0542
...
and never see the challenge (launch-audit finding H-1). Migration `0542` adds the
...
`0542` protects the settings mutators as the pattern. Broadly closing the API — reads of
driver PII, the privileged writes gated in `0541`, etc. — means adding
...
above if you want them gone). Server enforcement (0542) is separately reversible by
