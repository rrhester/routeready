...
const routes = {
  ...
  "/settings/avatar":       { render: renderAvatarStudio,    tab: "/profile", back: "/profile", title: "Edit Avatar" },
...
function avatarHtml(session, sizeClass) {
  ...
  const accent = avatarAccentColor(session);
  const ringStyle = accent ? `box-shadow:0 0 0 3px ${accent}, 0 0 0 5px rgba(255,255,255,.85)` : "";
  if (url) {
    return `<span class="${sizeClass} rr-av" style="background-image:url('${escapeHtml(url)}');background-size:cover;background-position:center;${ringStyle}"></span>`;
  }
  const bg = accent || "var(--accent)";
  return `<span class="${sizeClass} rr-av" style="background:${bg};${ringStyle}">${escapeHtml(initialsOf(name))}</span>`;
}

// Avatar metadata — per-device preferences that travel alongside the
// uploaded photo URL. Stored in localStorage so it survives reloads
// without needing a DB column. Read-only callers should treat the
// shape as opaque; only the Avatar Studio writes to it.
const AVATAR_META_KEY = "rr.driver.avatar_meta";
function readAvatarMeta() {
  try { return JSON.parse(localStorage.getItem(AVATAR_META_KEY) || "null") || {}; }
  catch { return {}; }
}
function writeAvatarMeta(m) {
  try { localStorage.setItem(AVATAR_META_KEY, JSON.stringify(m || {})); } catch {}
}
function avatarAccentColor(session) {
  const m = readAvatarMeta();
  // Only show the accent ring when the avatar actually belongs to
  // this driver's photo — switching accounts shouldn't leak colour
  // preferences across sessions.
  if (m.driver_id && session?.driver_id && m.driver_id !== session.driver_id) return null;
  return m.accent || null;
}
...
function renderProfileHub() {
  ...
  main.innerHTML = `
    <div class="home-hero">
      ...
      <button class="profile-avatar-btn" id="rr-photo-btn" type="button" aria-label="Edit avatar">
  ...
  // Tapping the avatar opens Avatar Studio — the unified surface for
  // uploading, generating, restyling, and accent-colouring the photo.
  document.getElementById("rr-photo-btn").addEventListener("click", () => {
    _haptic("tap");
...
function renderSettings() {
  ...
  const ICONS = {
    ...
    avatar:       '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="9" r="4"/><path d="M5.5 20a7 7 0 0 1 13 0"/><path d="M18 4l1.6 1.6M21 4l-1.6 1.6M19.6 6.4L21 7.8M19.6 3.6L18 5.2"/></svg>',
  ...
  main.innerHTML = `
    <div class="settings-page">
      <section class="settings-section">
        ...
        ${row("avatar",       "/settings/avatar",       "Edit avatar",  "Photo, AI or cartoon style, accent colour")}
...
// ── Avatar Studio ───────────────────────────────────────────────────
// Lets a driver personalise their profile photo three ways:
//   1. straight upload of a real photo
//   2. an AI-style render derived from that photo (Clean / Realistic /
//      Soft 3D / Illustrated)
//   3. a cartoon-style render derived from that photo (Cartoon Classic
//      / Fun & Bold / Minimal / Anime Style / Chibi / Vector Pop)
//
// Every variant is produced client-side by sampling the source photo
// into a canvas and re-quantising the pixel data with a style-specific
// pipeline (palette mapping, posterisation, edge passes, soft 3D
// gradient). Nothing leaves the device until the driver taps "Use This
// Avatar" — at which point we re-encode the canvas as a JPEG and ship
// it through the same upload-driver-photo edge function the bare-photo
// flow uses, so the avatar propagates everywhere the team roster /
// chat / dispatcher feed already read photos from.

// Style descriptors — order = grid order. `cat` groups them under the
// two top-level tabs. `kind` keys into _avatarRender so adding a new
// style is one entry here + one render branch.
const AVATAR_STYLES = [
  { id: "ai-clean",       cat: "ai",      label: "Clean",          sub: "Crisp · professional",     kind: "clean" },
  { id: "ai-realistic",   cat: "ai",      label: "Realistic",      sub: "Warm · natural",           kind: "realistic" },
  { id: "ai-soft3d",      cat: "ai",      label: "Soft 3D",        sub: "Plush · modern",           kind: "soft3d" },
  { id: "ai-illustrated", cat: "ai",      label: "Illustrated",    sub: "Hand-drawn feel",          kind: "illustrated" },
  { id: "cartoon-classic",cat: "cartoon", label: "Cartoon Classic",sub: "Bold lines · flat fills",  kind: "cartoon" },
  { id: "cartoon-fun",    cat: "cartoon", label: "Fun & Bold",     sub: "High-energy colour",       kind: "funbold" },
  { id: "cartoon-min",    cat: "cartoon", label: "Minimal",        sub: "Quiet · monochrome",       kind: "minimal" },
  { id: "cartoon-anime",  cat: "cartoon", label: "Anime Style",    sub: "Cel-shaded · cool tones",  kind: "anime" },
  { id: "cartoon-chibi",  cat: "cartoon", label: "Chibi",          sub: "Soft · approachable",      kind: "chibi" },
...
const AVATAR_ACCENTS = [
  { id: "none",    color: null,      label: "No accent" },
  { id: "azure",   color: "#2563EB", label: "Azure" },
  { id: "emerald", color: "#10B981", label: "Emerald" },
  { id: "violet",  color: "#8B5CF6", label: "Violet" },
  { id: "coral",   color: "#F97362", label: "Coral" },
  { id: "slate",   color: "#64748B", label: "Slate" },
];

// In-memory studio state — only lives while the page is mounted.
// `source` is the (possibly cropped) HTMLImageElement of the original
// upload; `originalUrl` is the dataURL we revert to. `previewBlob`
// caches the latest rendered output so saving is a single upload, not
// a re-render-then-upload.
let _avatarStudio = null;
...
async function renderAvatarStudio() {
  ...
  _avatarStudio = {
    activeCat: "ai",
    activeStyleId: meta.style_id || "ai-clean",
  ...
  // ("Edit Avatar"), and the back button is wired to /profile via the
  // route's `back` — no need to call setHeader here.
  main.innerHTML = avatarStudioShellHtml(session);
  _bindAvatarStudio(session);

  // If the driver already has a photo on file, seed the studio with
  // it so the first thing they see is their current avatar — not a
  // placeholder. The fetch is best-effort; failures fall back to the
  // empty-state copy on the preview tile.
  if (session?.photo_url) {
    try {
      const img = await _loadImageFromUrl(session.photo_url);
      _avatarStudio.source = img;
      _avatarStudio.sourceMime = "image/jpeg";
      _avatarStudioApplyStyle();
    } catch {
      // Silent fall-through — the upload tile still works.
    }
  }
}

function avatarStudioShellHtml(session) {
  const studio = _avatarStudio;
  const name   = session?.name || "Driver";
  const role   = session?.role || "Driver";
  const station = session?.station_code || session?.dsp_name || "—";
  const styleId = studio.activeStyleId;
  const active  = AVATAR_STYLES.find(s => s.id === styleId) || AVATAR_STYLES[0];

  const tabs = `
    <div class="av-tabs" role="tablist" aria-label="Avatar category">
      <button class="av-tab ${studio.activeCat === "ai" ? "is-active" : ""}" data-av-cat="ai"
              role="tab" aria-selected="${studio.activeCat === "ai"}">AI Avatars</button>
      <button class="av-tab ${studio.activeCat === "cartoon" ? "is-active" : ""}" data-av-cat="cartoon"
              role="tab" aria-selected="${studio.activeCat === "cartoon"}">Cartoon Avatars</button>
    </div>`;

  const grid = `
    <div class="av-style-grid" id="av-style-grid" role="listbox" aria-label="Choose a style">
      ${AVATAR_STYLES.filter(s => s.cat === studio.activeCat).map(s => `
  ...
  const accentSwatches = AVATAR_ACCENTS.map(a => {
    const sel = (studio.accent || null) === (a.color || null);
    const swatchStyle = a.color
      ? `background:${a.color}`
      : `background:linear-gradient(135deg, #fff 49%, #E5E7EB 49% 51%, #fff 51%);border:1.5px solid #E5E7EB`;
    return `
      <button class="av-accent ${sel ? "is-selected" : ""}" data-av-accent="${a.id}"
              aria-label="${escapeHtml(a.label)}" title="${escapeHtml(a.label)}"
              style="${swatchStyle}">
        ${sel ? `<span class="av-accent-tick" aria-hidden="true"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span>` : ""}
      </button>`;
  }).join("");
      ...
      <div class="av-hero">
        <div class="av-preview-wrap">
          <div class="av-preview" id="av-preview"
               style="${session?.photo_url ? `background-image:url('${escapeHtml(session.photo_url)}')` : ""}">
            ${session?.photo_url ? "" : `
              <span class="av-preview-fallback">${escapeHtml(initialsOf(name))}</span>`}
            <span class="av-preview-spinner" id="av-preview-spinner" hidden>
              <span class="loader" style="margin:0;width:24px;height:24px;border-width:3px"></span>
            </span>
          </div>
        </div>
        <div class="av-hero-copy">
          <div class="av-hero-title">Choose how you want to<br/>show up in RouteReady.</div>
        </div>

        <div class="av-primary-actions">
          <button class="av-primary av-primary-secondary" id="av-upload-btn" type="button">
          ...
          <button class="av-primary av-primary-primary" id="av-generate-btn" type="button">
            <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
            <span>Generate Avatar</span>
          </button>
        </div>
        <input type="file" id="av-file-input" accept="image/*" capture="user" style="display:none"/>
      </div>

      <div class="av-section">
        <div class="av-section-label">CHOOSE A STYLE</div>
        ${tabs}
        ${grid}
      </div>

      <div class="av-section">
        <div class="av-section-label">ACCENT COLOUR</div>
        <div class="av-accents" id="av-accents">${accentSwatches}</div>
      </div>

      <div class="av-section av-section-summary">
        <div class="av-summary-row">
          <span class="av-summary-key">Display Name</span>
          <span class="av-summary-val">${escapeHtml(name)}</span>
        </div>
        <div class="av-summary-row">
          <span class="av-summary-key">Role</span>
          <span class="av-summary-val">${escapeHtml(role)}</span>
        </div>
        <div class="av-summary-row">
          <span class="av-summary-key">Station</span>
          <span class="av-summary-val">${escapeHtml(station)}</span>
        </div>
      </div>

      <div class="av-actions-foot">
        <button class="btn btn-primary btn-block" id="av-save-btn" type="button">Save Avatar</button>
        <div class="av-actions-foot-row">
          <button class="av-text-btn" id="av-regenerate-btn" type="button">
          ...
          <button class="av-text-btn" id="av-revert-btn" type="button">
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg>
            Revert to original
          </button>
        </div>
      </div>
...
function _bindAvatarStudio(session) {
  const main = document.getElementById("main");
  ...
  fileInput.addEventListener("change", async (e) => {
    ...
    if (!/^image\//.test(file.type)) { toast("That doesn't look like an image.", "warn"); fileInput.value = ""; return; }
    try {
      ...
      _avatarStudio.activeStyleId = "ai-clean";
      _avatarStudioRefreshStyleGrid(session);
      _avatarStudioApplyStyle();
    } catch (err) {
      toast("Couldn't read that image. Try another.", "warn");
    } finally {
      fileInput.value = "";
    }
  });

  main.querySelectorAll(".av-tab").forEach(btn => {
    btn.addEventListener("click", () => {
      _avatarStudio.activeCat = btn.getAttribute("data-av-cat");
      // Reset to the first style in the new category so the grid
      // selection and the preview stay in sync.
      const first = AVATAR_STYLES.find(s => s.cat === _avatarStudio.activeCat);
      if (first) _avatarStudio.activeStyleId = first.id;
      _avatarStudioRefreshStyleGrid(session);
      _avatarStudioApplyStyle();
    });
  });
  ...
  main.querySelector("#av-accents").addEventListener("click", (e) => {
    const btn = e.target.closest("[data-av-accent]");
    if (!btn) return;
    const id = btn.getAttribute("data-av-accent");
    const accent = AVATAR_ACCENTS.find(a => a.id === id);
    if (!accent) return;
    _avatarStudio.accent = accent.color || null;
    main.querySelectorAll(".av-accent").forEach(s => s.classList.remove("is-selected"));
    btn.classList.add("is-selected");
    // Re-render so the tick lands on the new swatch and the preview
    // ring reflects the choice immediately.
  ...
  main.querySelector("#av-revert-btn").addEventListener("click", () => {
    if (!_avatarStudio.source) { toast("Nothing to revert to yet.", "warn"); return; }
    _avatarStudio.activeStyleId = "ai-clean";
    _avatarStudio.activeCat = "ai";
    _avatarStudio.accent = null;
    _avatarStudioRefreshStyleGrid(session);
    _avatarStudioApplyStyle();
    // Reflect accent reset in the swatch row without a full re-render.
    const accentBar = document.getElementById("av-accents");
    if (accentBar) {
      accentBar.querySelectorAll(".av-accent").forEach(s => s.classList.remove("is-selected"));
      const none = accentBar.querySelector(`[data-av-accent="none"]`);
      if (none) none.classList.add("is-selected");
    }
  });

  main.querySelector("#av-save-btn").addEventListener("click", async () => {
    await _avatarStudioSave(session);
  });

  _avatarStudioUpdatePreviewRing();
}

function _avatarStudioRefreshStyleGrid(session) {
  const wrap = document.getElementById("av-style-grid");
  const tabs = document.querySelectorAll(".av-tab");
  if (!wrap || !tabs.length) return;
  tabs.forEach(t => {
    const isActive = t.getAttribute("data-av-cat") === _avatarStudio.activeCat;
    t.classList.toggle("is-active", isActive);
    t.setAttribute("aria-selected", String(isActive));
  });
  wrap.innerHTML = AVATAR_STYLES.filter(s => s.cat === _avatarStudio.activeCat).map(s => `
    <button class="av-style-card ${s.id === _avatarStudio.activeStyleId ? "is-selected" : ""}"
...
function _avatarStudioBindStyleGrid(session) {
  document.querySelectorAll("[data-av-style]").forEach(card => {
    card.addEventListener("click", () => {
      const id = card.getAttribute("data-av-style");
      _avatarStudio.activeStyleId = id;
      document.querySelectorAll(".av-style-card").forEach(c => {
        const sel = c.getAttribute("data-av-style") === id;
...
// Bumps `renderToken` first so any in-flight async render becomes a
// no-op — keeps the preview in sync with the latest selection when
// the driver taps quickly between styles.
function _avatarStudioApplyStyle(opts) {
  const studio = _avatarStudio;
  ...
  const style = AVATAR_STYLES.find(s => s.id === studio.activeStyleId) || AVATAR_STYLES[0];
  const myToken = ++studio.renderToken;
  if (spinner) spinner.hidden = false;

  // Defer to the next frame so the spinner paints before the render
  // synchronously blocks the main thread.
  requestAnimationFrame(() => {
    if (studio.renderToken !== myToken) return;
    const dataUrl = _avatarRender(src, style.kind, {
      size: 512,
      seed: (opts && opts.jitter) ? Math.random() : null,
    });
    if (studio.renderToken !== myToken) return;
    studio.previewDataUrl = dataUrl;
    preview.style.backgroundImage = `url('${dataUrl}')`;
    // Drop the initials fallback once a real preview is mounted.
    const fb = preview.querySelector(".av-preview-fallback");
    if (fb) fb.remove();
...
function _avatarStudioUpdatePreviewRing() {
  const preview = document.getElementById("av-preview");
  if (!preview) return;
  const accent = _avatarStudio?.accent;
  preview.style.boxShadow = accent
    ? `0 0 0 5px ${accent}, 0 22px 60px -20px rgba(8,18,60,.45)`
...
async function _avatarStudioSave(session) {
  ...
  if (!studio.source || !studio.previewDataUrl) {
    toast("Upload a photo first.", "warn");
    return;
  }
  ...
  try {
    ...
    const styleSlug = (studio.activeStyleId || "avatar").replace(/[^a-z0-9-]/g, "");
    const fname = `${styleSlug}-${Date.now()}.jpg`;
    const file = new File([blob], fname, { type: "image/jpeg" });
    await uploadDriverPhoto(file);
    writeAvatarMeta({
      driver_id: session?.driver_id || null,
      style_id:  studio.activeStyleId,
      accent:    studio.accent || null,
      saved_at:  new Date().toISOString(),
    });
    // uploadDriverPhoto already toasts + re-renders; we navigate back
    // to the profile hub so the driver sees the new avatar on the
    // hero immediately.
    navigate("/profile");
  } catch (err) {
    toast(_friendlyError(err, "Couldn't save your avatar. Try again."), "warn");
  } finally {
...
// Each style is a deterministic transformation of the source image.
// Outputs a JPEG dataURL at the requested size, square-cropped.
// `seed` is a hint — used by styles that introduce tiny variation on
...
function _avatarRender(img, kind, opts) {
  ...
  // which reads more like a portrait than a centred crop.
  ...
  _avatarPreFill(ctx, size, kind, seed);

  ctx.save();
  // Draw to a rounded square inside the canvas — keeps the corners
  // sharp for the preview's circle clip but avoids losing pixels when
  // the canvas itself is reused as a thumbnail elsewhere.
  ctx.beginPath();
  ...
  // Apply the per-style pixel pass.
  let imageData;
  try { imageData = ctx.getImageData(0, 0, size, size); }
  catch { return null; } // cross-origin tainted — caller falls back to source
  _avatarPixelPass(imageData, kind, seed);
  ctx.putImageData(imageData, 0, 0);

  // Some styles add an overlay (vignette, glow, soft 3D inner shadow,
  // ink outline) after the pixel pass.
  _avatarOverlay(ctx, size, kind, seed);

  // Compression — JPEG quality tuned per style so flat-fill cartoons
  // don't waste bytes on imperceptible gradients.
  const q = (kind === "cartoon" || kind === "funbold" || kind === "pop" || kind === "minimal" || kind === "anime")
    ? 0.88 : 0.92;
  return c.toDataURL("image/jpeg", q);
}

function _avatarPreFill(ctx, size, kind, seed) {
  const tint = _avatarBgTint(kind, seed);
  if (!tint) {
    ctx.fillStyle = "#FFFFFF";
    ctx.fillRect(0, 0, size, size);
    return;
  }
  // Soft radial backdrop so flat-fill styles read as posed portraits.
  const grad = ctx.createRadialGradient(size * 0.5, size * 0.4, size * 0.15, size * 0.5, size * 0.55, size * 0.7);
...
function _avatarBgTint(kind, seed) {
  switch (kind) {
    case "cartoon": return { inner: "#FFF7E6", outer: "#FCE7B3" };
    case "funbold": return { inner: "#FFE6E6", outer: "#FCA5A5" };
    case "minimal": return { inner: "#F8FAFC", outer: "#E2E8F0" };
    case "anime":   return { inner: "#E8F0FF", outer: "#A5B8FF" };
    case "chibi":   return { inner: "#FFF0F4", outer: "#FBCFE8" };
    case "pop":     return { inner: "#FFF6D6", outer: "#FACC15" };
    case "soft3d":  return { inner: "#FBFCFF", outer: "#D9E4F5" };
    case "illustrated": return { inner: "#FFFCF5", outer: "#F0E3CB" };
    case "realistic":   return { inner: "#FFFAF3", outer: "#E9D8C2" };
    default:        return null;
  }
}

// Pixel passes — operate on the ImageData buffer in place.
function _avatarPixelPass(imageData, kind, seed) {
  const d = imageData.data;
  const n = d.length;
  switch (kind) {
    case "clean":      return _passClean(d, n);
    case "realistic":  return _passRealistic(d, n);
    case "soft3d":     return _passSoft3D(d, n, imageData.width, imageData.height);
    case "illustrated":return _passIllustrated(d, n, imageData.width, imageData.height);
    case "cartoon":    return _passCartoon(d, n, imageData.width, imageData.height);
    case "funbold":    return _passFunBold(d, n, imageData.width, imageData.height);
    case "minimal":    return _passMinimal(d, n);
    case "anime":      return _passAnime(d, n, imageData.width, imageData.height);
    case "chibi":      return _passChibi(d, n);
    case "pop":        return _passPop(d, n, imageData.width, imageData.height);
  }
}

// ── Clean — slight white-balance + soft contrast bump. The default
// "real photo" treatment for drivers who want their actual face.
function _passClean(d, n) {
  for (let i = 0; i < n; i += 4) {
    d[i]   = _clip(_curve(d[i],   1.04, 8));
    d[i+1] = _clip(_curve(d[i+1], 1.04, 6));
    d[i+2] = _clip(_curve(d[i+2], 1.02, 4));
  }
}

// ── Realistic — warm skin tones, slight saturation lift.
function _passRealistic(d, n) {
  for (let i = 0; i < n; i += 4) {
...
// ── Soft 3D — slight blur via posterise + warm highlight + cool
// shadow shift. Reads as a plush 3D render at avatar scale.
function _passSoft3D(d, n) {
  for (let i = 0; i < n; i += 4) {
    let r = d[i], g = d[i+1], b = d[i+2];
    // Posterise to 24 levels — flattens skin into rendered planes.
    r = Math.round(r / 11) * 11;
    g = Math.round(g / 11) * 11;
    b = Math.round(b / 11) * 11;
    // Shadow → cool, highlight → warm — the classic CG lighting tell.
    const lum = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
    if (lum > 0.6) { r += 14; g += 8; }
    else if (lum < 0.35) { b += 10; r -= 4; }
    d[i] = _clip(r); d[i+1] = _clip(g); d[i+2] = _clip(b);
  }
}

// ── Illustrated — heavy posterise + edge softening, like a marker
// portrait. Strong dark accents and an off-white paper tint.
function _passIllustrated(d, n) {
  for (let i = 0; i < n; i += 4) {
    let r = d[i], g = d[i+1], b = d[i+2];
    ...
    const lum = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
    if (lum < 0.28) { r *= 0.7; g *= 0.7; b *= 0.7; }
    // Subtle paper warm-up
    d[i]   = _clip(r * 0.97 + 8);
    d[i+1] = _clip(g * 0.97 + 4);
    d[i+2] = _clip(b * 0.94);
  }
}

// ── Cartoon Classic — flat fills + dark ink line. Posterises hard
// and pulls saturation up so skin reads as cartoon skin, not photo.
function _passCartoon(d, n) {
  for (let i = 0; i < n; i += 4) {
    let r = d[i], g = d[i+1], b = d[i+2];
    r = Math.round(r / 48) * 48;
    g = Math.round(g / 48) * 48;
    b = Math.round(b / 48) * 48;
    const avg = (r + g + b) / 3;
    r = avg + (r - avg) * 1.35;
    g = avg + (g - avg) * 1.25;
    b = avg + (b - avg) * 1.20;
...
// ── Fun & Bold — very high saturation, lifted shadows, comic-style
// punch.
function _passFunBold(d, n) {
  for (let i = 0; i < n; i += 4) {
    let r = d[i], g = d[i+1], b = d[i+2];
    ...
    g = avg + (g - avg) * 1.45;
    b = avg + (b - avg) * 1.40;
    // Lift shadows so the face reads bright and friendly.
    if ((r + g + b) / 3 < 90) { r += 18; g += 14; b += 12; }
    d[i] = _clip(r); d[i+1] = _clip(g); d[i+2] = _clip(b);
  }
}

// ── Minimal — desaturated portrait, gentle contrast. Reads like a
// black-and-white press shot with a hint of warm grey.
function _passMinimal(d, n) {
  for (let i = 0; i < n; i += 4) {
    const lum = 0.299 * d[i] + 0.587 * d[i+1] + 0.114 * d[i+2];
    const tone = _clip(_curve(lum, 1.08, -4));
    d[i]   = _clip(tone * 1.02);
    d[i+1] = _clip(tone * 1.00);
    d[i+2] = _clip(tone * 0.96);
  }
}

// ── Anime — cel-shaded look: hard luminance bands, cool tint, ink
// edges land via the overlay pass.
function _passAnime(d, n) {
  for (let i = 0; i < n; i += 4) {
    let r = d[i], g = d[i+1], b = d[i+2];
    const lum = 0.299 * r + 0.587 * g + 0.114 * b;
    // Three discrete tones: shadow / midtone / highlight.
    const band = lum < 80 ? 0.55 : lum < 170 ? 0.85 : 1.05;
    r = r * band; g = g * band; b = b * band;
    // Cool shadow shift — slight blue / violet.
    if (lum < 110) { b += 18; r -= 4; }
    d[i] = _clip(r); d[i+1] = _clip(g); d[i+2] = _clip(b);
  }
}

// ── Chibi — soft pastel skin, lifted highlights, very gentle.
function _passChibi(d, n) {
  for (let i = 0; i < n; i += 4) {
    let r = d[i], g = d[i+1], b = d[i+2];
    r = Math.round(r / 28) * 28;
    g = Math.round(g / 28) * 28;
    b = Math.round(b / 28) * 28;
    // Slight pink wash everywhere — chibi reads warm and friendly.
    d[i]   = _clip(r * 0.94 + 22);
    d[i+1] = _clip(g * 0.92 + 14);
    d[i+2] = _clip(b * 0.96 + 12);
  }
}

// ── Vector Pop — pop-art quantise to a small palette.
function _passPop(d, n) {
  const palette = [
    [255, 230, 80],  [255, 132, 88], [240, 80, 110],
    [80, 132, 232],  [20, 26, 70],   [255, 255, 250],
  ];
  for (let i = 0; i < n; i += 4) {
    const r = d[i], g = d[i+1], b = d[i+2];
    let best = palette[0], dist = 1e9;
...
// Overlay pass — runs after putImageData so it draws above the pixel
// transform. Used for vignette / ink outline / 3D inner shadow.
function _avatarOverlay(ctx, size, kind, seed) {
  if (kind === "cartoon" || kind === "anime" || kind === "pop" || kind === "funbold") {
    _avatarInkOutline(ctx, size, kind);
  }
  if (kind === "soft3d" || kind === "realistic" || kind === "chibi") {
    // Soft inner glow — sells the "rendered" feel without obscuring.
    const grad = ctx.createRadialGradient(size * 0.5, size * 0.5, size * 0.32, size * 0.5, size * 0.5, size * 0.52);
    grad.addColorStop(0, "rgba(255,255,255,0)");
    grad.addColorStop(1, kind === "chibi" ? "rgba(251,207,232,.32)" : "rgba(8,18,60,.18)");
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, size, size);
  }
  if (kind === "minimal" || kind === "illustrated") {
    // Subtle paper-edge vignette.
    const grad = ctx.createRadialGradient(size * 0.5, size * 0.5, size * 0.4, size * 0.5, size * 0.5, size * 0.62);
    grad.addColorStop(0, "rgba(0,0,0,0)");
    grad.addColorStop(1, "rgba(15,23,42,.18)");
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, size, size);
  }
}

// Quick edge pass via a Sobel-ish sample of the rendered canvas.
// Operates on a downsampled copy so it stays fast at 512² on phones.
function _avatarInkOutline(ctx, size, kind) {
  const w = size;
  const h = size;
  ...
  const od = out.data;
  const sd = src.data;
  // Sample every 2 pixels — ink lines stay readable, work halves.
  const step = 2;
  const threshold = kind === "anime" ? 70 : kind === "pop" ? 90 : 60;
  for (let y = step; y < h - step; y += step) {
    for (let x = step; x < w - step; x += step) {
      ...
      const lum = (p) => 0.299 * sd[p] + 0.587 * sd[p+1] + 0.114 * sd[p+2];
      const gx = Math.abs(lum(ir) - lum(il));
      const gy = Math.abs(lum(idn) - lum(iu));
      const g = gx + gy;
      if (g > threshold) {
        // Stamp a 2x2 dark block — gives the line a hand-inked weight
        // without doing a true per-pixel pass.
  ...
  // Composite the ink layer over the source.
...
// ── Pixel helpers ───────────────────────────────────────────────────
function _clip(v) { return v < 0 ? 0 : v > 255 ? 255 : v | 0; }
function _curve(v, contrast, shift) {
  // S-curve approximation: centre on 128, scale by contrast.
  return (v - 128) * contrast + 128 + shift;
}
...
function _loadImageFromUrl(url) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    // CORS hint — the driver-photos bucket is public so this should
    // succeed; without it the getImageData call later would throw on
    // a tainted canvas.
    img.crossOrigin = "anonymous";
    img.onload  = () => resolve(img);
    img.onerror = () => reject(new Error("image_load_failed"));
    img.src = url + (url.includes("?") ? "&" : "?") + "rrcav=" + Date.now();
  });
}
