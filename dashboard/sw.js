...
//   2026-07-08.74 · Schedule week board accessibility: the driver grid now carries WAI-ARIA grid semantics (role=grid/row/columnheader/rowheader/gridcell + per-cell aria-labels), a single roving tabindex with Arrow/Home/End keyboard navigation across day cells, and a keyboard-only assign flow — focus an open-shift chip, press Enter to "pick it up", navigate to a driver-day cell, and press Enter to assign (Escape cancels). Actions are announced via an aria-live region. A keyboard/screen-reader operator can now schedule without a mouse; previously drag-and-drop was the only path. live.js (_rrA11yStampScheduleGrid + keyboard handler in bindSchedWeekNav) + schedule-rrx.css focus/carry styles.
const SW_DEPLOY_NONCE = "2026-07-08.74";
