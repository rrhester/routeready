...
async function _renderOkamiDailyPanelImpl(weekIdx) {
  ...
  container.innerHTML = `
    <div class="okami-daily-panel" style="grid-template-columns:1fr">
      ...
      <details class="okami-week-settings" data-rr-okami-settings="${weekIdx}" style="grid-column:1 / -1;margin-top:14px;border-top:1px solid var(--border);padding-top:10px">
        <summary style="cursor:pointer;font-size:10px;font-weight:600;color:var(--text-subtle);user-select:none;letter-spacing:.04em;text-transform:uppercase">Week settings</summary>
        <div class="okami-week-settings-body" style="margin-top:10px;font-size:13px;color:var(--text-subtle)">Loading…</div>
      </details>
    </div>`;

  bindOkamiDailyDelegation();
  bindOkamiWeekSettingsToggle(weekIdx, fmtIsoDate(weekStart));
}

// First-open lazy load + edit/save for the per-week Settings disclosure.
// Pure addition: the Schedule view's settings drawer remains the canonical
// editor. This is a planning-time convenience so the operator can tweak
// cushion + waves while looking at OKAMI demand for that same week.
function bindOkamiWeekSettingsToggle(weekIdx, weekStartIso) {
  const det = document.querySelector(`[data-rr-okami-settings="${weekIdx}"]`);
  if (!det) return;
  let loaded = false;
  det.addEventListener("toggle", async () => {
    if (!det.open || loaded) return;
    loaded = true;
    await loadOkamiWeekSettings(det, weekStartIso);
  });
}

async function loadOkamiWeekSettings(det, weekStartIso) {
  const body = det.querySelector(".okami-week-settings-body");
  if (!body) return;
  const { data, error } = await sb.rpc("scheduling_settings_for_week", { p_week_start: weekStartIso });
  if (error) {
    body.innerHTML = `<div style="color:var(--red);font-size:12px">Couldn't load settings: ${escapeHtml(error.message)}</div>`;
    return;
  }
  const s = data || {};
  const cushion = s.cushion_pct ?? 10;
  const waves = (Array.isArray(s.waves) && s.waves.length) ? s.waves : [{ start: "07:00" }];
  const inheritedNote = s.is_inherited
    ? `<span style="color:var(--text-subtle);font-size:11px">Inherited from a previous week — saving makes this week's settings independent.</span>`
    : "";

  const wavesHtml = waves.map((w, i) => `
    <div data-rr-ows-wave style="display:flex;gap:6px;align-items:center">
      <input type="time" class="form-input" data-rr-ows-wave-time value="${escapeHtml(w.start || "07:00")}" style="max-width:130px"/>
      ${waves.length > 1 ? `<button type="button" class="btn btn-sm" data-rr-ows-remove-wave style="color:var(--red)">Remove</button>` : ""}
    </div>`).join("");

  body.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
      <div>
        <div style="font-size:11px;font-weight:600;color:var(--text-muted);letter-spacing:.04em;text-transform:uppercase;margin-bottom:6px">Cushion</div>
        <div style="display:flex;align-items:center;gap:8px">
          <input type="number" min="0" max="50" step="1" class="form-input" data-rr-ows-cushion value="${cushion}" style="width:90px"/>
          <span style="font-size:11px;color:var(--text-subtle)">% extra above OKAMI</span>
        </div>
      </div>
      <div>
        <div style="font-size:11px;font-weight:600;color:var(--text-muted);letter-spacing:.04em;text-transform:uppercase;margin-bottom:6px">Wave start times</div>
        <div data-rr-ows-waves style="display:flex;flex-direction:column;gap:6px;margin-bottom:6px">${wavesHtml}</div>
        <button type="button" class="btn btn-sm" data-rr-ows-add-wave>+ Add wave</button>
      </div>
      <div style="grid-column:1 / -1;display:flex;align-items:center;gap:10px;border-top:1px solid var(--border);padding-top:10px;margin-top:4px">
        <button class="btn btn-primary btn-sm" data-rr-ows-save data-week="${weekStartIso}">Save week settings</button>
        <span data-rr-ows-status style="font-size:11px;color:var(--text-subtle)"></span>
        ${inheritedNote}
      </div>
      <div style="grid-column:1 / -1;font-size:11px;color:var(--text-subtle)">For block length, max-days-per-week, service types and availability override use the full <a href="#" data-rr-ows-open-schedule style="color:var(--accent-text)">Schedule settings</a>.</div>
    </div>`;
}

document.addEventListener("click", async (e) => {
  // Add wave row inside an OKAMI week settings disclosure.
  const addBtn = e.target.closest("[data-rr-ows-add-wave]");
  if (addBtn) {
    e.preventDefault();
    const wrap = addBtn.previousElementSibling;
    if (!wrap) return;
    const last = wrap.querySelector("[data-rr-ows-wave]:last-child [data-rr-ows-wave-time]");
    let next = "07:00";
    if (last?.value) {
      const [h, m] = last.value.split(":").map(n => parseInt(n, 10));
      const total = h * 60 + m + 25;
      next = `${String(Math.floor(total / 60) % 24).padStart(2, "0")}:${String(total % 60).padStart(2, "0")}`;
    }
    wrap.insertAdjacentHTML("beforeend", `
      <div data-rr-ows-wave style="display:flex;gap:6px;align-items:center">
        <input type="time" class="form-input" data-rr-ows-wave-time value="${escapeHtml(next)}" style="max-width:130px"/>
        <button type="button" class="btn btn-sm" data-rr-ows-remove-wave style="color:var(--red)">Remove</button>
      </div>`);
    return;
  }
  const remBtn = e.target.closest("[data-rr-ows-remove-wave]");
  if (remBtn) {
    e.preventDefault();
    remBtn.closest("[data-rr-ows-wave]")?.remove();
    return;
  }
  const openSched = e.target.closest("[data-rr-ows-open-schedule]");
  if (openSched) {
    e.preventDefault();
    if (typeof goto === "function") goto("schedule");
    return;
  }
  const saveBtn = e.target.closest("[data-rr-ows-save]");
  if (!saveBtn) return;
  e.preventDefault();
  const det = saveBtn.closest("[data-rr-okami-settings]");
  if (!det) return;
  const weekStart = saveBtn.dataset.week;
  const status = det.querySelector("[data-rr-ows-status]");
  const cushion = parseInt(det.querySelector("[data-rr-ows-cushion]")?.value, 10) || 0;
  const waves = Array.from(det.querySelectorAll("[data-rr-ows-wave-time]"))
  ...
  try {
    const { error: upErr } = await sb.rpc("upsert_scheduling_settings", {
      p_payload: { week_start: weekStart, cushion_pct: cushion, waves, timezone: tz },
    });
    if (upErr) throw upErr;
    if (status) status.textContent = "Saved · regenerating shifts…";
    const { error: regErr } = await sb.rpc("regenerate_week_shifts", { p_week_start: weekStart });
    if (regErr) throw regErr;
    await sb.rpc("apply_cushion_to_week", { p_week_start: weekStart }).catch(() => {});
    if (status) { status.style.color = "var(--green)"; status.textContent = `Saved at ${new Date().toLocaleTimeString()}`; }
    toast("Week settings saved", "success");
    if (typeof renderOkamiLive === "function") renderOkamiLive();
  } catch (err) {
    if (status) { status.style.color = "var(--red)"; status.textContent = "Failed: " + (err?.message || err); }
    toast("Save failed: " + (err?.message || err), "warn");
  } finally {
    saveBtn.disabled = false;
  }
});
