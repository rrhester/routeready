-- ─────────────────────────────────────────────────────────────────────────
-- Migration 0284 · Overtime Intelligence — forward-looking projection
--
-- This is an overtime tracking tool, not a worked-vs-scheduled report.
-- Up to 0283 the function computed OT exclusively from `worked_hours`,
-- which meant a driver scheduled for 50 hours sat at 0 projected OT
-- until they actually checked in for 41 hours — at which point the
-- exposure was already incurred.  That defeats the purpose of an
-- "intelligence" surface: by the time it lit up, the OT was unstoppable.
--
-- New model: every per-driver row now carries a `projected_hours`
-- value = worked-to-date (from real check-ins) PLUS remaining
-- scheduled hours that are still attainable.  OT hours + risk
-- classification fire off the projection, not the actuals — so a van
-- whose schedule alone breaches 40h surfaces immediately, before any
-- wheels turn.
--
-- Per-shift projected_hrs rule:
--   1. NCNS / called_off / VTO       → 0    (attendance loss, gone)
--   2. checked_in + checked_out      → real worked time (precise)
--   3. checked_in, no check-out yet  → scheduled block hours
--                                       (driver showed up; assume they
--                                       finish the planned block)
--   4. no check-in, end_time < now   → 0    (past shift, never started)
--   5. otherwise (future / in-prog)  → scheduled block hours
--
-- Per-driver projected_hours = sum of per-shift projected_hrs across
-- the worked-status set (scheduled / completed / late).  NCNS, VTO and
-- called-off shifts contribute 0 via rule 1.
--
-- `worked_hours` is unchanged from 0283 (real check-ins only).  The
-- table now exposes both: Worked = what we observed, Projected = what
-- we expect by week's end.  Variance remains worked − scheduled
-- (definitional; it answers a different question).
--
...
begin
  ...
  with shift_rows as (
    select
      ...
      -- eff_in / eff_out drive the observed-only `worked_hrs` column.
      -- Real check-ins only — see 0283.
      case
        when s.status::text = any(v_worked_statuses) then c.checked_in_at
      end as eff_in,
      case
        when s.status::text = any(v_worked_statuses)
             and c.checked_in_at is not null then
          coalesce(
            c.checked_out_at,
            case when s.ends_at is not null then least(v_now, s.ends_at) end
          )
      end as eff_out
  ...
  shift_calc as (
    select
      r.*,
      -- Worked hours: only real check-in time.  Honest observed count.
      case
      ...
      -- Projected hours: worked-to-date + remaining scheduled, with
      -- attendance gaps zeroed out.  See per-shift rules in the
      -- migration header above.
      case
        when r.status in ('no_show','called_off','vto') then 0
        when r.checked_in_at is not null
             and r.checked_out_at is not null
             and r.checked_out_at > r.checked_in_at then
          extract(epoch from (r.checked_out_at - r.checked_in_at)) / 3600.0
        when r.checked_in_at is not null then
          r.scheduled_hrs                        -- in progress / missed checkout
        when r.ends_at is not null and r.ends_at < v_now then
          0                                       -- past shift, never started
        else
          r.scheduled_hrs                        -- future / in-progress, no check-in yet
      end as projected_hrs,
      -- Rescue hours track observed, not projected — they describe
      -- coverage that already happened.
      case
  ...
  per_driver as (
    select
      ...
      sum(sc.projected_hrs)     as projected_hrs,
  ...
  scored as (
    select
      ...
      coalesce(pd.projected_hrs, 0)::numeric      as projected_hours,
  ...
  enriched as (
    -- OT projection + risk are now driven by `projected_hours`, not by
    -- `worked_hours`, so a 50-hour schedule lights up as 10h projected
    -- OT today — before any check-ins land.
    select
      s.*,
  ...
  select
    coalesce(jsonb_agg(
      jsonb_build_object(
        ...
        'projected_hours',       round(e.projected_hours, 2),
    ...
    count(*) filter (where e.projected_hours >= v_threshold),
    coalesce(sum(e.rescue_hours), 0),
    coalesce(sum(e.worked_hours), 0),
    coalesce(sum(e.projected_hours), 0),
  ...
  -- Same projection model — share OT across route groups by each
  -- driver's planned-hour weighting.
  with route_rows as (
    select
      s.driver_id,
      coalesce(
  ...
  driver_totals as (
    -- Per-driver projected hours over the threshold = projected OT.
    -- Mirrors the per-shift projected_hrs rule above so route
    -- attribution lines up with the per-driver table.
    select
      s.driver_id,
      greatest(
        0,
        sum(
          case
            when s.status::text in ('no_show','called_off','vto') then 0
            when c.checked_in_at is not null
                 and c.checked_out_at is not null
                 and c.checked_out_at > c.checked_in_at then
              extract(epoch from (c.checked_out_at - c.checked_in_at)) / 3600.0
            when c.checked_in_at is not null then
              coalesce(s.block_hours::numeric,
                       case when s.starts_at is not null and s.ends_at is not null
                            then extract(epoch from (s.ends_at - s.starts_at)) / 3600.0
                            else 10 end)
            when s.ends_at is not null and s.ends_at < v_now then 0
            else
              coalesce(s.block_hours::numeric,
                       case when s.starts_at is not null and s.ends_at is not null
                            then extract(epoch from (s.ends_at - s.starts_at)) / 3600.0
  ...
  -- Prior week is finalized — projection collapses to "what actually
  -- happened" so the rule reduces to real check-ins + status-zero loss.
  ...
  return jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'total_projected_hours', round(coalesce(v_total_projected, 0), 1),
