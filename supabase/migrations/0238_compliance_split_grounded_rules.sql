-- Migration 0238 · Split grounded-vehicle exceptions into separate
-- rule violations + add a "14 BD overdue" red bucket.
--
-- Old behavior (0235/0236): every grounded VIN emitted ONE risk row
-- with severity flipped between yellow/red based on RO + BD age.
-- DSPs lose signal — a vehicle that's grounded > 14 BD AND missing
-- its RO is two different Amazon-DSP rule violations and should
-- surface as two separate exception rows so each one can be cleared
-- (and audit-trailed) individually.
--
-- New kinds:
--   · grounded_no_ro       — vehicle is grounded with no active RO.
--                            Yellow ≤ 2 BD (you're still inside the
--                            "open RO within 2 BD" window).
--                            Red > 2 BD (window missed).
--
--   · repair_14bd_overdue  — vehicle has been grounded > 14 BD,
--                            regardless of RO state.  Red.  Amazon's
--                            "complete repair within 14 BD" rule.
--
-- A single VIN can fire both at once (e.g., grounded 18 BD with no
-- RO).  Each surfaces as its own row in the Exceptions table.
--
-- Idempotent.
...
declare
  ...
  v_fmcsa jsonb; v_docs jsonb; v_audit jsonb; v_monitors jsonb;
  v_fleet jsonb; v_risks jsonb; v_gaps jsonb;
...
begin
  ...
  -- ── Rule 1 · "Open a repair order within 2 business days of grounding"
  --     Fires for every grounded vehicle that has no active RO.
  v_risks := v_risks || coalesce((
    ...
    where (g->>'ro_id') is null
  ), '[]'::jsonb);

  -- ── Rule 2 · "Complete repair within 14 business days of grounding"
  --     Fires whenever a vehicle has been grounded > 14 BD, regardless
  --     of RO state (a missing RO doesn't excuse a missed completion).
  v_risks := v_risks || coalesce((
    select jsonb_agg(jsonb_build_object(
    ...
    where (g->>'bd_grounded')::numeric > 14
  ), '[]'::jsonb);
  ...
  if v_fmcsa is not null and (v_fmcsa->>'cure_required')::boolean then
    v_risks := v_risks || jsonb_build_array(jsonb_build_object(
      ...
      'subjects', jsonb_build_array(jsonb_build_object('lbl','USDOT','val', coalesce(v_fmcsa->>'usdot','—'))),
      'meta', jsonb_build_object(
  ...
  return jsonb_build_object(
    ...
    'fmcsa', v_fmcsa, 'documents', v_docs, 'audit_events', v_audit,
