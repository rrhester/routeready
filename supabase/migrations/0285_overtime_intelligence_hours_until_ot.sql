-- ─────────────────────────────────────────────────────────────────────────
-- Migration 0285 · Overtime Intelligence — hours until OT + per-day drill
--
-- Two operator-facing additions, no behavioural change to the existing
-- projection / OT model from 0284:
--
--   1. `hours_until_ot` (per driver) = greatest(0, threshold − worked).
--      The forward-looking "non-overtime capacity remaining" — when this
--      is low and a driver is still scheduled for a 10-hour shift, the
--      operator can intervene (send home early, rescue, swap) BEFORE
--      hours roll into OT.
--
--   2. `days[]` (per driver) — one entry per scheduled shift this week,
--      sorted by date.  Drives the click-through "by-day report" the
--      driver-detail drawer renders on the dashboard.  Each entry
--      carries the per-shift `scheduled_hours`, `worked_hours`, and
--      `projected_hours` (same projection rule as the rollup), plus
--      shift identifiers (status, kind, route, start/end, check-in
--      timestamps) so the dashboard can paint a complete daily view
--      without a second round-trip.
--
-- Worked/projected definitions are unchanged from 0284.  Idempotent.
-- ─────────────────────────────────────────────────────────────────────────
...
begin
  ...
  per_driver as (
    select
      ...
      -- Per-day breakdown that powers the driver-detail drawer's
      -- "by-day report".  One row per scheduled shift, in date order.
      jsonb_agg(
        jsonb_build_object(
          'shift_id',        sc.shift_id,
          'date',            sc.date,
          'status',          sc.status,
          'shift_kind',      sc.shift_kind,
          'route_code',      sc.route_code,
          'starts_at',       sc.starts_at,
          'ends_at',         sc.ends_at,
          'scheduled_hours', round(sc.scheduled_hrs::numeric, 2),
          'worked_hours',    round(sc.worked_hrs::numeric, 2),
          'projected_hours', round(sc.projected_hrs::numeric, 2),
          'checked_in_at',   sc.checked_in_at,
          'checked_out_at',  sc.checked_out_at
        ) order by sc.date, sc.starts_at
      ) as days_json
    from shift_calc sc
  ...
  scored as (
    select
      ...
      coalesce(pd.days_json, '[]'::jsonb)         as days
  ...
  enriched as (
    select
      ...
      -- Non-overtime capacity remaining = how many more hours the
      -- driver can work this week BEFORE crossing the OT threshold.
      -- Driven off WORKED (not projected) so it reflects "right now",
      -- not "by the end of the week".
      greatest(0, v_threshold - worked_hours)::numeric as hours_until_ot,
      case
  ...
  select
    coalesce(jsonb_agg(
      jsonb_build_object(
        ...
        'hours_until_ot',        round(e.hours_until_ot,  2),
  ...
  -- ── 2. Top OT-creating route group (unchanged from 0284) ───────────────
  with route_rows as (
    select
      s.driver_id,
      coalesce(
  ...
  -- ── 3. Prior-week OT hours for trend (unchanged from 0284) ─────────────
