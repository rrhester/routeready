-- ─────────────────────────────────────────────────────────────────────────
-- Migration 0282 · Overtime Intelligence — don't project future shifts
--
-- Bug: on the first day of a week (or any day with shifts still ahead),
-- the dashboard's "Worked" column already showed the full scheduled
-- block hours, because the eff_in / eff_out fallbacks used
-- `starts_at - 10 minutes` / `ends_at` for every shift in status
-- ('scheduled','completed','late'). For a future 'scheduled' shift this
-- treated the whole planned block as if it were already worked, which
-- also pushed projected OT up before any wheels turned.
--
-- Fix: clamp the fallback times to now(). When the schedule says the
-- shift hasn't started yet, eff_in collapses to now() and eff_out
-- collapses to now(), so worked_hrs computes to 0. When the shift is
-- in progress, eff_out = now() so worked_hrs reflects only the elapsed
-- portion. Past shifts are unaffected — ends_at is already < now().
--
-- Real check-in / check-out rows from driver_checkins are still used
-- verbatim (those are observed times, not projections).
--
-- Same fix applied to the route-group OT attribution block so the
-- "top OT route" matches the per-driver table.
--
-- The prior-week block is left as-is — its window is always in the
-- past, so the clamp would be a no-op there.
--
...
begin
  ...
  with shift_rows as (
    select
      ...
      --
      -- Fallbacks (starts_at − 10m / ends_at) are clamped to now() so
      -- future scheduled shifts don't count as already-worked. Real
      -- check-in / check-out timestamps are used verbatim.
      case
        when s.status::text = any(v_worked_statuses) then
          coalesce(
            c.checked_in_at,
            case when s.starts_at is not null
                 then least(v_now, s.starts_at - interval '10 minutes') end
          )
      end as eff_in,
      case
        when s.status::text = any(v_worked_statuses) then
  ...
  -- route-group attribution lines up with the table totals. Fallback
  -- times are clamped to now() to match the per-driver block above.
  with route_rows as (
    select
      s.driver_id,
      coalesce(
  ...
  driver_totals as (
    select
      ...
      greatest(0,
        sum(
          case
            when coalesce(c.checked_in_at,  least(v_now, s.starts_at - interval '10 minutes')) is not null
             and coalesce(c.checked_out_at, least(v_now, s.ends_at)) is not null
             and coalesce(c.checked_out_at, least(v_now, s.ends_at))
                 > coalesce(c.checked_in_at, least(v_now, s.starts_at - interval '10 minutes'))
  ...
  -- Prior week is always fully in the past, so the now()-clamp is a
  -- no-op — left unclamped to keep the diff small.
