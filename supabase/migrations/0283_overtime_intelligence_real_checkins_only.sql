-- ─────────────────────────────────────────────────────────────────────────
-- Migration 0283 · Overtime Intelligence — count hours only when there's
--                                            a real driver check-in
--
-- Symptom: even when *no* driver checked in via the app, every scheduled
-- shift whose start time had passed reported ~10 worked hours.  For a
-- standard 10-hour block that translated to exactly 10.2 worked hours
-- per driver (10h block + 10 min pre-shift slack), which then powered
-- bogus variance + projected-OT figures on the Overtime Intelligence
-- dashboard.
--
-- Root cause: the eff_in fallback introduced in 0279 / extended in
-- 0282 — `starts_at - 10 minutes` — treated every "scheduled / late /
-- completed" shift as if the driver had clocked in, regardless of
-- whether a `driver_checkins` row actually existed.  Migration 0282
-- clamped the fallback to now() so future shifts wouldn't accrue
-- imaginary hours, but it left the past-shift case intact.  This
-- migration removes the fallback entirely.
--
-- New rule (applied consistently across all three blocks below):
--
--   1. eff_in  = c.checked_in_at exclusively.  No time-based fallback.
--      If a driver never checked in, no worked hours accrue — period.
--
--   2. eff_out = real check-out when present.  When the driver checked
--      in but hasn't checked out yet (or the check-out was missed),
--      project elapsed time up to least(now(), ends_at) so an
--      in-progress shift still counts.  This branch only fires when
--      eff_in is real.
--
--   3. NCNS / called_off / VTO still contribute 0 worked hours (they
--      can't reach this code path — status is filtered out earlier).
--
-- Side-effect: DSPs that haven't rolled out the driver app's check-in
-- flow will see 0 worked hours across the board.  That's the intent —
-- it's the honest answer to "how many hours did we observe drivers
-- working?"  Surface the fallback nowhere; let the dashboard guide
-- operators toward the data they need.
--
...
declare
  ...
  -- Statuses where the driver could have accrued worked hours.  NCNS,
  -- VTO, and called-off are excluded so they never reach worked_hrs.
...
begin
  ...
  with shift_rows as (
    select
      ...
      -- eff_in: real check-in only.  No time-based fallback — a missing
      -- check-in means the driver never clocked in, so no worked hours
      -- accrue.  Worked-status gate keeps NCNS / VTO / call-off out of
      -- this branch.
      case
        when s.status::text = any(v_worked_statuses) then c.checked_in_at
      end as eff_in,
      -- eff_out: real check-out, or project elapsed time up to
      -- least(now(), ends_at) when the driver checked in but hasn't
      -- checked out yet.  Gated on a real check-in so we never
      -- manufacture worked hours from thin air.
      case
        when s.status::text = any(v_worked_statuses)
          ...
          coalesce(
            ...
            case when s.ends_at is not null
                 then least(v_now, s.ends_at) end
          )
      end as eff_out
  ...
  -- Recompute per-driver OT using the same real-check-in rule so the
        ...
        sum(
          case
            when c.checked_in_at is not null
             and coalesce(c.checked_out_at, least(v_now, s.ends_at)) is not null
             and coalesce(c.checked_out_at, least(v_now, s.ends_at)) > c.checked_in_at
              then extract(epoch from (
                     coalesce(c.checked_out_at, least(v_now, s.ends_at))
                     - c.checked_in_at
                   )) / 3600.0
            else 0
          end
  ...
  -- Same real-check-in rule applies — prior weeks without app adoption
  -- now read 0 OT instead of projecting fake hours.
        ...
        sum(
          case
            when s.status::text = any(v_worked_statuses)
             ...
             and coalesce(c.checked_out_at, s.ends_at) > c.checked_in_at
              then extract(epoch from (
