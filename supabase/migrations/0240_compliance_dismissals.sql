-- Migration 0240 · Compliance · dismiss-for-14-days
--
-- An operator can dismiss a surfaced exception row.  Each dismissal
-- is recorded with an expires_at timestamp 14 days out.  The bundle
-- filters out any exception that has a non-expired dismissal for the
-- same (kind, object_id) pair, so the row disappears from the
-- Exceptions table.  When the dismissal expires, the next sweep
-- re-emits the exception if the underlying state still warrants it.
--
-- The audit row is permanent — we want a paper trail of every
-- "I acknowledged this and chose to snooze it" action.
--
-- Idempotent.

-- ── 1. compliance_dismissals ────────────────────────────────────────
create table if not exists public.compliance_dismissals (
  id              uuid primary key default gen_random_uuid(),
  dsp_id          uuid not null references public.dsps(id) on delete cascade,
  exception_kind  text not null,                  -- 'grounded_no_ro' | 'repair_14bd_overdue' | …
  object_id       text,                            -- as it appears in the bundle (string form)
  reason          text,
...
create policy "compliance_dismissals_rw" on public.compliance_dismissals
  for all to authenticated
  using (dsp_id = private.current_dsp_id())
  with check (dsp_id = private.current_dsp_id());
grant select, insert, delete on public.compliance_dismissals to authenticated;


-- ── 2. compliance_dismiss(p_kind, p_object_id, p_days?) ─────────────
...
-- ── 3. compliance_workspace_bundle · filter dismissed risks ─────────
-- Wraps the v_risks final assignment so dismissed (kind, object_id)
-- pairs drop out of the surfaced array.  Everything else in the
-- bundle is unchanged from migration 0238.
