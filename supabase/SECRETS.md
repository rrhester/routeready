...
| `TREMENDOUS_OAUTH_CLIENT_ID` | `tremendous-oauth-start`, `tremendous-oauth-callback`, `send-driver-reward` (token refresh) | From Tremendous → Developers → Applications → register a new app named "RouteReady". Required for the per-DSP OAuth handshake (migration 0303). Without it, the "Connect Tremendous" button in Settings → Rewards returns `oauth_not_configured`. |
| `TREMENDOUS_OAUTH_CLIENT_SECRET` | `tremendous-oauth-callback`, token refresh | Paired with the client ID above. Tremendous shows it once on app creation — store it as a Supabase secret immediately. |
| `TREMENDOUS_OAUTH_REDIRECT_URI` | `tremendous-oauth-start`, `tremendous-oauth-callback` | Must match the redirect URI configured on the Tremendous OAuth app. Recommended value: `https://doiwrhkirgblcvuskhno.functions.supabase.co/tremendous-oauth-callback`. |
| `PUBLIC_BASE_URL` | `tremendous-oauth-callback` (already required by `webhook-twilio`) | Used to construct the per-DSP webhook URL we register on the DSP's Tremendous account during the OAuth handshake. Set to `https://doiwrhkirgblcvuskhno.functions.supabase.co`. |
| `TREMENDOUS_OAUTH_AUTHORIZE_URL` | `tremendous-oauth-start` | Optional override. Defaults to Tremendous's production authorize endpoint. Override only if Tremendous renames it or you're hitting sandbox. |
| `TREMENDOUS_OAUTH_TOKEN_URL` | `tremendous-oauth-callback`, token refresh | Optional override. Defaults to Tremendous's production token endpoint. |
| `TREMENDOUS_OAUTH_SCOPE` | `tremendous-oauth-start` | Optional override. Defaults to `rewards funding_sources webhooks`. Match this to the scopes your Tremendous OAuth app requests. |
| `TREMENDOUS_API_BASE` | `send-driver-reward` (Tremendous provider) | Optional override. Defaults to production (`https://api.tremendous.com/api/v2`). Flip to `https://testflight.tremendous.com/api/v2` for sandbox testing. |
| `TREMENDOUS_WEBHOOK_SECRET` | `webhook-tremendous` (legacy fallback only) | Legacy single-tenant fallback. In the per-DSP OAuth model, each DSP's webhook secret is stored on `dsp_reward_integrations.webhook_secret`. Set this only if you ran the single-tenant version of the migration (`0302`) before adopting OAuth. |
| `DASHBOARD_REWARDS_RETURN_URL` | `tremendous-oauth-callback` | Optional. Fallback URL to redirect to if the original return path is missing from the OAuth state. Defaults to `https://gorouteready.com/dashboard/#/settings/rewards`. |
