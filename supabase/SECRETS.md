...
| `RESEND_EVENTS_WEBHOOK_SECRET` | `webhook-email-events` | The `whsec_…` signing secret for the *deliverability-events* webhook (delivered / bounced / complained). Set up in Resend → Webhooks → Add endpoint → URL `https://doiwrhkirgblcvuskhno.functions.supabase.co/webhook-email-events`, events `email.delivered`, `email.bounced`, `email.complained`, `email.delivery_delayed`. Falls back to `RESEND_WEBHOOK_SECRET` if you point the *same* Resend endpoint at both functions (not recommended — separate endpoints get separate secrets). Without it every POST 500s (`events_secret_missing`). A bounced quote-request email flips its Repair Center request to **failed** (red in the queue) — this secret is what turns silent non-delivery into a visible state. |
